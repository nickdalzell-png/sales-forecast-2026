...
# Pacing data
pacing = pd.DataFrame({
    "Date": ["Oct 1", "Oct 15", "Nov 1", "Nov 15", "Dec 1", "Dec 15", "Jan 1", "Jan 15", "Feb 1", "Feb 15", "Mar 1", "Mar 15"],
    "Cumulative $": [656000, 800000, 861000, 1210000, 1300000, 2370000, 3930000, 4350000, 4650000, 4920000, 5310000, 5960000],
    "% of Goal": [7, 8, 9, 12, 12, 23, 37, 41, 44, 47, 51, 57]
})

# YoY quarterly data (split to avoid any truncation)
...
yoy_quarterly = pd.DataFrame({
    "Region": yoy_region,
    "Period": yoy_period,
    "2025 Actual": yoy_2025,
    "2026 Actual": yoy_2026,
    "Cash Growth / Variance": yoy_cash
})
...
regions = st.sidebar.multiselect("Choose regions", options=regional["Region"].tolist(), default=regional["Region"].tolist())
...
with tab1:
    ...
    fig_q = px.bar(quarterly, x="Quarter", y=["2026
